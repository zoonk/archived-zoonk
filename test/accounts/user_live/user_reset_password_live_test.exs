defmodule ZoonkWeb.UserResetPasswordLiveTest do
  ...
  describe "Reset password page" do
    ...
    test "does not render reset password with invalid token", %{conn: conn} do
      ...
      assert to == %{flash: %{"error" => "Reset password link is invalid or it has expired."}, to: ~p"/"}
    end

    test "renders errors for invalid data", %{conn: conn, token: token} do
      {:ok, lv, _html} = live(conn, ~p"/users/reset_password/#{token}")

      result = lv |> element("#reset_password_form") |> render_change(user: %{"password" => "secret", "confirmation_password" => "secret123456"})
  ...
  describe "Reset Password" do
    test "resets password once", %{conn: conn, token: token, user: user} do
      ...
      {:ok, conn} =
        lv
        |> form("#reset_password_form", user: %{"password" => "ValidPassword1", "password_confirmation" => "ValidPassword1"})
        |> render_submit()
    ...
    test "does not reset password on invalid data", %{conn: conn, token: token} do
      ...
      result =
        lv
        |> form("#reset_password_form", user: %{"password" => "short", "password_confirmation" => "does not match"})
        |> render_submit()
