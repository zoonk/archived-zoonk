...
defmodule UneebeeWeb.Components.Avatar do
  ...
  def avatar(%{src: nil} = assigns) do
    ~H"""
    <div class={avatar_class(@size, @class)} title={@alt}><%= avatar_label(@alt) %></div>
    """
  end
  ...
  defp avatar_class(size, class) do
    [
      "shrink-0 rounded-full uppercase bg-primary text-white text-xs object-cover flex items-center justify-center flex-column font-semibold",
      size == :small && "h-5 w-5",
      size == :medium && "h-8 w-8",
